...
# -*- coding: utf-8 -*-
# @Date: 2020-06-30
# @File: weblogic_jdbc.py
# @Author: zhangwei
# @Desc: monitor weblogic jdbc
...
if __name__ == "__main__":
    ...
    # easy_port = '7040'
    easy_data_name = 'lis_weblogic_jdbc'
    ...
    if login:
        ...
        for j in jdbc:
            ...
            if 'MySQLDB_CORE' in jdbc:
                continue

            jdbc_perf = wc.perf_jdbc(parent=domain, name=jdbc)
            for server, vals in jdbc_perf.items():
                dims = {
                    'data_name': easy_data_name, 'domain': domain,
                    'server': str(server), 'jdbc': jdbc
                }
                data = {'dims': dims, 'vals': vals}
